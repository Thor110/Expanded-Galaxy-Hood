...
void main() {
	...
	if ((GetTag(oEntering) == "bos_walker")) {
		...
		AssignCommand(oBos_walkthing, ActionDoCommand(DestroyObject(oBos_walkthing, 0.0, 0, 0.0)));
		AssignCommand(oEntering, ActionDoCommand(SetFacing(180.0)));
		AssignCommand(oEntering, ActionWait(5.0));
		AssignCommand(oEntering, ActionPlayAnimation(112, 1.0, 0.0));

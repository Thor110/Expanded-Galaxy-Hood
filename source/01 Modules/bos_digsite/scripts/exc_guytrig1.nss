...
void main() {
	...
	if ((GetIsObjectValid(oExc_prisoner1) == 1)) {
		if ((oExc_torture == GetEnteringObject())) {
			AssignCommand(oExc_torture, ActionDoCommand(SetCommandable(0, oExc_torture)));
			AssignCommand(oExc_torture, ClearAllActions());
			AssignCommand(oExc_torture, SetFacingPoint(GetPosition(oExc_prisoner1)));
			AssignCommand(oExc_torture, ActionDoCommand(ActionWait(1.0)));
			AssignCommand(oExc_prisoner1, ActionDoCommand(ActionWait(1.0)));
			effect efDamage = EffectDamage(100, 512, 5);
			effect efBeam = EffectBeam(2038, oExc_prisoner1, 0, 0);
			effect effect5 = EffectBeam(2038, oExc_torture, 3, 0);
			string string1 = "n_twilekm_hit1";
			PlaySound(string1);
			AssignCommand(oExc_torture, ActionCastFakeSpellAtObject(35, oExc_torture, 0));
			DelayCommand(0.2, AssignCommand(oExc_torture, ApplyEffectToObject(1, effect5, GetObjectByTag("exc_prisoner1", 0), 1.0)));
			DelayCommand(0.2, AssignCommand(oExc_prisoner1, ActionPlayAnimation(21, 1.0, 3.0)));
			DelayCommand(0.2, AssignCommand(oExc_prisoner1, ApplyEffectToObject(0, EffectVisualEffect(1021, 0), oExc_prisoner1, 0.0)));
			DelayCommand(0.2, AssignCommand(oExc_prisoner1, ApplyEffectToObject(0, efDamage, oExc_prisoner1, 0.0)));
			AssignCommand(oExc_torture, ActionDoCommand(ActionWait(1.0)));
			AssignCommand(oExc_torture, ActionDoCommand(SetFacing(270.0)));
			AssignCommand(oExc_torture, ActionDoCommand(SetCommandable(1, oExc_torture)));
			AssignCommand(oExc_torture, ActionDoCommand(sub1()));
		}
	}
}

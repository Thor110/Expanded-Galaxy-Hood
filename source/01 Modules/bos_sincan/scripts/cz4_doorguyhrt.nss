...
int sub2(object objectParam1) {
	...
	if ((GetTag(OBJECT_SELF) == "DEBUG")) {
		sub3(OBJECT_SELF);
		sub4(("Attempted Attack Target = " + IntToString(GetIsObjectValid(object1))), 10, 20, 4.0);
		sub4(("Attempted Spell Target = " + IntToString(GetIsObjectValid(object3))), 10, 22, 4.0);
		sub4(("Attack Target = " + IntToString(GetIsObjectValid(oTarget))), 10, 24, 4.0);
		sub4(("Move Target = " + IntToString(GetIsObjectValid(object7))), 10, 26, 4.0);
	}
...
void main() {
	...
	object oTem_rakata = GetObjectByTag("tem_rakata", 0);
	object oTem_rakata2 = GetObjectByTag("tem_rakata2", 0);
	object oDesert_tukata = GetObjectByTag("desert_tukata", 0);
	object oNPC = GetPartyMemberByIndex(0);
	if ((GetIsObjectValid(oTem_lol) == 0)) {
		if ((GetStandardFaction(OBJECT_SELF) == 4)) {
			object oNearest = GetNearestCreature(7, 7, OBJECT_SELF, 1, 3, 1, 0xFFFFFFFF, 0xFFFFFFFF);
			if (((!sub1(intGLOB_103)) && (!GetSoloMode()))) {
				if ((GetPartyMemberByIndex(0) != OBJECT_SELF)) {
					if (((((GetCurrentAction(OBJECT_SELF) != 36) && (!GetIsConversationActive())) && (!sub1(intGLOB_102))) && GetCommandable(OBJECT_SELF))) {
						if ((((!sub2(OBJECT_SELF)) && (!GetIsObjectValid(oNearest))) && (GetDistanceBetween(OBJECT_SELF, GetPartyMemberByIndex(0)) < 5.0))) {
							ExecuteScript("k_ai_master", OBJECT_SELF, 1005);
						}
						else {
							ClearAllActions();
							if ((GetDistanceBetween(OBJECT_SELF, GetPartyMemberByIndex(0)) < 5.0)) {
								ClearAllActions();
								ActionForceMoveToObject(GetPartyMemberByIndex(0), 0, 3.0, 30.0);
							}
							if ((GetDistanceBetween(OBJECT_SELF, GetPartyMemberByIndex(0)) > 5.0)) {
								ClearAllActions();
								ActionForceMoveToObject(GetPartyMemberByIndex(0), 1, 3.5, 30.0);
							}
						}
					}
				}
				...
				if ((GetSoloMode() && (GetCurrentAction(OBJECT_SELF) == 38))) {
					ClearAllActions();
				}
			}
			if (sub1(intGLOB_82)) {
			...
			if ((GetDistanceBetween(OBJECT_SELF, oNPC) > 5.0)) {
				...
				ActionMoveAwayFromObject(oNPC, 1, 10.0);
				DelayCommand(1.0, ActionMoveAwayFromObject(oNPC, 1, 10.0));
			}
		}
	}
	if ((GetIsObjectValid(oTem_lol) == 1)) {

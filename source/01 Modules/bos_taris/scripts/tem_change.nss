...
void main() {
	...
	AssignCommand(oMr_starter, ActionDoCommand(DestroyObject(oMr_starter, 0.0, 0, 0.0)));
	AddAvailableNPCByObject(0, oTaris_channa);
	SwitchPlayerCharacter(0);
	SetGlobalFadeIn(1.0, 2.0, 0.0, 0.0, 0.0);
	CreateObject(64, "tem_lol", Location(Vector(195.04, 95.15, 90.0), 0.0), 0);
	sub1(GetFirstPC(), GetObjectByTag("bos_storage", 0));
	DelayCommand(0.5, ExecuteScript("tem_equip", oArea, 0xFFFFFFFF));

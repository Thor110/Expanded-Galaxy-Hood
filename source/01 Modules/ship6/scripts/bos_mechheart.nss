...
if (GetDistanceBetween(OBJECT_SELF, GetFirstPC()) > 16.0)
    {
ActionDoCommand(SetFacing(160.0));
    }
...
SetFacingPoint(GetPosition(GetFirstPC()));
    ...
    if (!GetLocalBoolean(OBJECT_SELF, 130)) {
        ...
        SetLocalBoolean(OBJECT_SELF, 130, TRUE);
    }
    else {
        SetLocalBoolean(OBJECT_SELF, 130, FALSE);
    }
    ...
    if (!GetLocalBoolean(OBJECT_SELF, 150)) {
        ...
        SetLocalBoolean(OBJECT_SELF, 150, TRUE);
    }
    else {
        SetLocalBoolean(OBJECT_SELF, 150, FALSE);
    }
    ...
    if (!GetLocalBoolean(OBJECT_SELF, 160)) {
        ...
        SetLocalBoolean(OBJECT_SELF, 160, TRUE);
    }
    else {
        SetLocalBoolean(OBJECT_SELF, 160, FALSE);
    }

    if (!GetLocalBoolean(OBJECT_SELF, 170)) {
        DelayCommand(1.0, SetFacingPoint(GetPosition(GetFirstPC())));
        SetLocalBoolean(OBJECT_SELF, 170, TRUE);
    }
    else {
        SetLocalBoolean(OBJECT_SELF, 170, FALSE);
    }

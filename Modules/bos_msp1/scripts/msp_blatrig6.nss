...
void main() {
	...
	if ((oNPC == GetEnteringObject())) {
		...
		object object6 = CreateItemOnObject("g_w_lngswrd01", oMsp_cpuguy, 1);
		string string1 = "n_mndlorian_bat2";
		PlaySound(string1);
		AssignCommand(oMsp_cpuguy, SetFacingPoint(GetPosition(GetFirstPC())));
		AssignCommand(oMsp_cpuguy, ActionPlayAnimation(108, 1.0, 0.0));
		AssignCommand(oMsp_cpuguy, ChangeToStandardFaction(GetObjectByTag("msp_cpuguy", 0), 1));
		AssignCommand(oMsp_cpuguy, ActionEquipItem(object6, 4, 0));
		AssignCommand(oMsp_cpuguy, sub1(OBJECT_INVALID));

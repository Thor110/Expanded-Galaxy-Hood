...
void main() {
	...
	if ((int2 == 1006)) {
		...
		if ((nCurHP < 10)) {
			...
			object object13 = GetObjectByTag("bastila", 0);
			...
			CancelCombat(oMission);
			CancelCombat(oT3M4);
			CancelCombat(oZaalbar);
			CancelCombat(object13);
			ClearAllActions();
			ClearAllEffects();
			...
			SetSoloMode(1);
			SetLightsaberPowered(object13, 0, 0, 0);
			SetGlobalFadeOut(3.0, 2.0, 0.0, 0.0, 0.0);
			DelayCommand(5.0, AssignCommand(oMrs_invis, ActionStartConversation(GetFirstPC(), "bos_channa2", 0, 0, 0, "", "", "", "", "", "", 0)));
		}
	}
}

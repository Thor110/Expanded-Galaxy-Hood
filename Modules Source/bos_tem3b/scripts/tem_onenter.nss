...
void main() {
	...
	if (((oPC == GetEnteringObject()) && (GetIsObjectValid(oTem_end2) == 1))) {
		object oTem_mechanic = GetObjectByTag("tem_mechanic", 0);
		object oWst_charlie = GetObjectByTag("wst_charlie", 0);
		object oOrn_kobayashi = GetObjectByTag("orn_kobayashi", 0);
		CreateObject(1, "wjt_charlie2", Location(Vector((-0.09), (-2.63), 0.0), 270.0), 0);
		vector struct3 = Vector(0.11, 19.99, 6.08);
		location location2 = Location(struct3, 270.0);
		vector struct5 = Vector((-0.76), 20.96, 6.08);
		location location4 = Location(struct5, 0.0);
		SpawnAvailableNPC(1, location2);
		CreateObject(1, "tjm_mechanic", location4, 0);
		object oTem_end = GetObjectByTag("tem_end", 0);
		AssignCommand(oTem_end, ActionStartConversation(GetFirstPC(), "tem_kobalive", 0, 0, 1, "", "", "", "", "", "", 0));
		object object26 = GetObjectByTag("tem_end2", 0);
		AssignCommand(object26, ActionDoCommand(DestroyObject(object26, 0.0, 0, 0.0)));
		AssignCommand(oPC, JumpToLocation(Location(Vector((-0.69), 18.43, 0.0), 90.0)));
		...
		DelayCommand(1.0, AssignCommand(oOrn_kobayashi, ActionPlayAnimation(30, 1.0, 9999.0)));
	}
}
